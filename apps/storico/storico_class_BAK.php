...
class storicoApp extends appBaseClass {
    ...
    function __construct($param,$galileo) {
        ...
        foreach ( $this->gruppo['oggettiActual'] as $oggetto=>$o) {
            $tempstr.="'".$oggetto."',";
            $this->gruppo['eventi'][$oggetto]=array();
        }
                ...
                while ($row=$this->galileo->getFetchPiattaforma($map['piattaforma'],$fid)) {
                    ...
                    if ($row['dms']=='concerto') {
                        ...
                        if ($row['num_doc']=='0') {
                            ...
                            $this->pratiche[$row['pratica']]['prenotazione']['obj']->setStorico(true);
                            ...
                            $this->pratiche[$row['pratica']]['commesse'][$row['rif']]['obj']->setStorico(true);
        ...
        foreach ($this->pratiche as $pratica=>$p) {
            if (isset($p['commesse'])) {
                ...
                foreach ($p['righe'] as $oggetto=>$o) {
                    $this->gruppo['eventi'][$oggetto][]=$p['info'];
                }
            }
        }
    ...
    function normalizzaDeltaEventi() {
        ...
        foreach ($this->gruppo['eventi'] as $oggetto=>$o) {
            ...
            foreach ($o as $k=>$c) {
                ...
                if ($t && $c['d_rif'] && $t!="" && $c['d_rif']!="") $this->gruppo['eventi'][$oggetto][$k]['deltat']=round(mainFunc::gab_delta_tempo($t,$c['d_rif'],'g')/30);
                else $this->gruppo['eventi'][$oggetto][$k]['deltat']=false;

                //if ($km && $c['km'] && $c['km']!=0) $this->gruppo['eventi'][$oggetto][$k]['deltakm']=$c['km']-$km;
                //else $this->gruppo['eventi'][$oggetto][$k]['deltakm']=false;

                $this->gruppo['eventi'][$oggetto][$k]['deltakm']=$c['km']-$km;

                //adegua record precedente
                if ($k>0) {
                    if ($this->gruppo['eventi'][$oggetto][$k-1]['d_rif'] && $c['d_rif'] && $this->gruppo['eventi'][$oggetto][$k-1]['d_rif']!="" && $c['d_rif']!="") $this->gruppo['eventi'][$oggetto][$k-1]['deltat']=round(mainFunc::gab_delta_tempo($c['d_rif'],$this->gruppo['eventi'][$oggetto][$k-1]['d_rif'],'g')/30);
                    else $this->gruppo['eventi'][$oggetto][$k-1]['deltat']=false;

                    if ($this->gruppo['eventi'][$oggetto][$k-1]['km'] && $c['km'] && $this->gruppo['eventi'][$oggetto][$k-1]['km']!="" && $c['km']!=0) $this->gruppo['eventi'][$oggetto][$k-1]['deltakm']= $this->gruppo['eventi'][$oggetto][$k-1]['km']-$c['km'];
                    else $this->gruppo['eventi'][$oggetto][$k-1]['deltakm']=false;
                }
    ...
    function drawBody() {
            ...
            echo '<div style="position:relative;display:inline-block;width:35%;height:100%;padding:3px;box-sizing:border-box;//border-right:1px solid black;vertical-align:top;">';
                ...
                echo '<div style="position:relative;height:10%;" >';
                    ...
                    echo '<div style="position:relative;display:inline-block;width:70%;vertical-align:top;" >';
                        ...
                        elseif ($this->gruppo['codice']!="" && isset($this->gruppi[$this->gruppo['codice']])) {
                            /*
                            ...
                            */
                            ...
                            echo '<div style="font-size:1.2em;border: 1px solid black;padding:2px;border-radius:10px;text-align:center;background-color:thistle;" >'.$this->gruppo['descrizione'].'</div>';
                    ...
                    echo '<div style="position:relative;display:inline-block;width:30%;vertical-align:top;text-align:center;" >';
                        ...
                        if (isset($this->actualVei['rif']) && $this->actualVei['cod_marca']!="" && $this->actualVei['modello']!="") {
                            echo '<img style="width:25px;height:25px;margin-top:10px;cursor:pointer;" src="http://'.$_SERVER['SERVER_ADDR'].'/nebula/apps/storico/img/edit.png" onclick="window[\'_nebulaApp_\'+window._nebulaApp.getTagFunzione()].selectPiano(\''.$this->actualVei['cod_marca'].'\',\''.$this->actualVei['modello'].'\',\''.$this->gruppo['codice'].'\');" />';
                        }
        ...
        echo '<script type="text/javascript" >';
            ...
            if (isset($this->gruppo['oggettiActual'])) {
                echo 'var base="'.base64_encode(json_encode($this->base)).'";';
                echo 'var actual="'.base64_encode(json_encode($this->gruppo['oggettiActual'])).'";';
            }
            else {
            ...
            foreach ($this->pratiche as $k=>$p) {
                ...
                echo '<div style="position:relative;margin-top:10px;">';
                    ...
                    if (isset($p['manuale'])) $this->drawPassMan($p);
                        ...
                        echo '<div style="position:relative;border-left:2px solid black;border-top:2px solid black;width:30%;overflow-x:visible;padding:2px;box-sizing:border-box;" >';
                        ...
                        echo '<div style="position:relative;border-left:2px solid black;width:100%;padding:2px;box-sizing:border-box;">';
                        ...
                        echo '<div style="position:relative;border-top:2px solid black;width:100%;padding:2px;box-sizing:border-box;width:95%;">';
    ...
    function drawPrevisione() {
        ...
        echo '<div style="position:relative;width:100%;height:10%;border-bottom:1px solid black;" >';
            ...
            if ($this->gruppo['codice']=="") $error.=' - Nessun piano di manutenzione';
            ...
            if ($error!="") {
                ...
                $dCons=(isset($this->actualVei['d_cons']) && $this->actualVei['d_cons']!="")?$this->actualVei['d_cons']:( (isset($this->parami['sto_consegna']) && $this->param['sto_consegna']!="")?$this->param['sto_consegna']:"");
                ...
                echo '<div style="position:relative;display:inline-block;width:50%;vertical-align:top;margin-top:3px;" >';
                    ...
                    foreach ($this->gruppo['eventi'] as $codice=>$c) {
                        foreach ($c as $k=>$e) {
                            $temp[$codice]=array(
                                'd_rif'=>$e['d_rif'],
                                'km'=>$e['km']
                ...
                echo '<div style="position:relative;display:inline-block;width:49%;vertical-align:top;margin-top:3px;" >';
                    ...
                    echo '<div style="position:relative;display:inline-block;width:90%;vertical-align:top;margin-left:3%;">';
                        ...
                        echo '<div id="sto_prev_mese" style="position:relative;display:inline-block;width:40%;height:20px;vertical-align:top;border:1px solid black;text-align:center;font-weight:bold;background-color: #ecf1a5;"></div>';
                    echo '</div>'; 
