...
class c2rBudget_S {
    ...
    protected $catBonus=array();
    protected $bonus=array();
    protected $totBonus=array();
    ...
    function build() {
        ...
        foreach ($allReparti as $rep=>$r) {
            ...
            $this->griglia[$rep]['diretto']=0;
            $this->griglia[$rep]['indiretto']=0;
            /*$this->griglia[$rep]['primo']=array(
                "valore"=>0,
                "extra"=>0
            );
            $this->griglia[$rep]['secondo']=array(
                "valore"=>0,
                "extra"=>0
            );
            $this->griglia[$rep]['terzo']=array(
                "valore"=>0,
                "extra"=>0
            );
            $this->griglia[$rep]['finale']=array(
                "valore"=>0,
                "extra"=>0
            );*/
        }
        ...
        $this->griglia['tot']['oneri']=array(
            "valore"=>0,
            "costi"=>0
        );
        $this->griglia['tot']['diretto']=0;
        $this->griglia['tot']['indiretto']=0;
        ...
        $this->param['costoDefInd']=json_decode('{"gar":18,"bdc":18,"mag":18,"lav":16,"gen":17}',true);
        $this->param['costoDefDir']=json_decode('{"TEC":19,"CAR":20,"RT":21,"RS":21,"RC":21,"altro":18}',true);
        ...
        $row=array(
            array(
                ...
                "data_i"=>'20200101',
                "data_f"=>'21001231',
                "mano"=>'{"TEC":19,"RT":21,"RS":21,"RC":21,"CAR":20,"altro":18}'
            ),
            array(
                "reparto"=>"AUS",
                "data_i"=>'20200101',
        ...
        */
        if (array_key_exists('mac',$this->fissi)) {
            foreach ($this->prodLinkRif['common'] as $rep=>$r) {
                ...
                foreach ($this->reparti as $krep=>$reparto) {
                    ...
                    $tempTOTpres+=(isset($this->param['prodLink']['prestato'][$rep][$reparto]['presenza']['valori']['prestato']['valore']))?$this->param['prodLink']['prestato'][$rep][$reparto]['presenza']['valori']['prestato']['valore']:0;
                }
                ...
                foreach ($r['presMat'] as $kpres=>$p) {
                    foreach ($this->reparti as $krep=>$reparto) {
                        ...
                        $this->log[]=array($rep,$reparto,$val,$prc,$costo);
        ...
        $this->catBonus=array(
            ...
            "pro"=>array(
                "tag"=>"Azioni Promozionali",
                "valore"=>0
            )
        );

        foreach ($this->reparti as $rep=>$r) {
            $this->bonus[$rep]=$this->catBonus;
            $this->totBonus[$rep]=0;
        }
        $this->totBonus['totale']=0;
        ...
        foreach ($trow as $tk=>$v) {
            ...
            //se il periodo del record è interamente contenuto nel periodo di analisi prendilo senza calcoli
            //if ($v['data_f']<=$this->param['fine'] && $v['data_i']>=$this->param['inizio']) {
            if (array_key_exists($v['cat'],$this->catBonus)) {

                if (isset($this->bonus[$v['reparto']][$v['cat']])) {
                    $rowGG=mainfunc::gab_delta_tempo($v['data_i'],$v['data_f'],'g')+1;
                    $intGG=mainfunc::gab_delta_tempo( ($v['data_i']<$this->param['inizio'])?$this->param['inizio']:$v['data_i'],($v['data_f']>$this->param['fine'])?$this->param['fine']:$v['data_f'],'g')+1;

                    $this->bonus[$v['reparto']][$v['cat']]['valore']=($rowGG==0)?0:$v['bonus']*($intGG/$rowGG);
                }
            }
        }
    ...
    function drawDiretti() {
        ...
        */
        ...
        echo '<div style="width:100%;height:100%;overflow:scroll;" >';
            ...
            //echo json_encode($this->presDiretti);
                ...
                echo '<tbody>';
                    ...
                    foreach ($this->gruppiIndiretti as $gruppo=>$g) {
                        ...
                        echo '<tr style="font-weight:bold;">';
                            ...
                            echo '<td style="text-align:center;">'.number_format($tempTot,2,'.','').'</td>';
            ...
            echo '<table style="border-collapse:collapse;font-size:10pt;width: max-content;margin-bottom:20px;margin-right:50px;" >';
                ...
                echo '<tbody>';
                        ...
                        echo '">';
                            ...
                            foreach ($this->reparti as $rep=>$r) {
                                ...
                                echo '<td style="text-align:right;';
                                    //se non viene sommato al totale del contesto
                                ...
                                echo '" >';
                                    if ($self) {
                                        ...
                                        $this->griglia[$rep]['oneri']['costi']+=$this->raggCosti[$rep][$gruppo]['costi'];
                                    }
                            ...
                            $this->griglia['tot']['oneri']['costi']+=$temp;
                    ...
                    foreach (c2rBlocco::$fattClassi as $k=>$c) {
                        ...
                        echo '<tr>';
                            ...
                            foreach ($this->reparti as $rep=>$r) {
                                ...
                                echo '</td>';
                                $temp+=$val;
                                $this->griglia[$rep]['oneri']['valore']+=$val;
                            }
                            echo '<td style="text-align:right;">'.number_format($temp,0,'','.').'</td>';
                            $this->griglia['tot']['oneri']['valore']+=$temp;
                        echo '</tr>';

                        //compensa valori che non vengono fatturati nel DMS nel modo giusto
                                ...
                                echo '<tr style="color:violet;font-size:0.9em;" >';
                                    ...
                                    foreach ($this->reparti as $rep=>$r) {
                                        if (isset($compensation[$rep][$k])) {
                                            ...
                                            $this->griglia[$rep]['oneri']['valore']+=$compensation[$rep][$k];
                    ...
                    echo '<tr style="border-top: 2px solid black;border-bottom: 2px solid black;">';
                        echo '<th style="text-align:left;height:40px;">Margine Oneri:</th>';
                        ...
                        foreach ($this->reparti as $rep=>$r) {
                            ...
                            echo '<th style="text-align:right;';
                                if (!$tempColor[$rep]) {
                                    echo 'color:orange;" >('.number_format($this->griglia[$rep]['oneri']['valore']-$this->griglia[$rep]['oneri']['costi'],0,'','.').')';
                                }
                                else {
                                    echo '" >'.number_format($this->griglia[$rep]['oneri']['valore']-$this->griglia[$rep]['oneri']['costi'],0,'','.');
                                }
                            echo '</th>';
                        }
                        echo '<th style="text-align:right;" >'.number_format($this->griglia['tot']['oneri']['valore']-$this->griglia['tot']['oneri']['costi'],0,'','.').'</th>';
                    echo '</tr>';
                    ...
                    echo '<tr style="border-top: 2px solid black;border-bottom: 2px solid black;">';
                        ...
                        foreach ($this->reparti as $rep=>$r) {
                            ...
                            echo '<th style="text-align:right;';
                                if (!$tempColor[$rep]) {
                                    echo 'color:orange;" >'.number_format($this->griglia[$rep]['fatt'],0,'','.');
                                }
                                else {
                    ...
                    echo '<tr style="background-color:#ffdf61;" >';
                        echo '<th style="text-align:left;height:20px;">1° Margine:</th>';
                        //foreach ($this->griglia as $rep=>$r) {
                            //if ($rep=='tot') continue;
                        foreach ($this->reparti as $rep=>$r) {
                            $this->griglia[$rep]['primo']=$this->griglia[$rep]['oneri']['valore']-$this->griglia[$rep]['oneri']['costi']+$this->griglia[$rep]['fatt']-$this->griglia[$rep]['lis']['costo']-$this->griglia[$rep]['lis']['excos']-$this->griglia[$rep]['ric']['gesmag'];
                            echo '<th style="text-align:right;" >'.number_format($this->griglia[$rep]['primo'],0,'','.').'</th>';
                        }
                        $this->griglia['tot']['primo']=$this->griglia['tot']['oneri']['valore']-$this->griglia['tot']['oneri']['costi']+$this->griglia['tot']['fatt']-$this->griglia['tot']['variabili'];
                        echo '<th style="text-align:right;" >'.number_format($this->griglia['tot']['primo'],0,'','.').'</th>';
                    echo '</tr>';
                    ...
                    echo '<tr>';
                        ...
                        foreach ($this->reparti as $rep=>$r) {
                            ...
                            $this->griglia[$rep]['diretto']+=$costo;
                        }
                        echo '<td style="text-align:right;" >'.number_format($temp,0,',','.').'</td>';
                        $this->griglia['tot']['diretto']+=$temp;
                    echo '</tr>';
                    ...
                    echo '<tr style="font-size:0.9em;color:violet;" >';
                        ...
                        foreach ($this->reparti as $rep=>$r) {
                            ...
                            //se il reparto NON è common
                            if (!array_key_exists($rep,$this->prodLinkRif['common'])) {
                                $this->griglia[$rep]['diretto']+=$dnp[$rep]['distr'];
                            }
                            $temp+=$dnp[$rep]['distr'];
                        }
                        echo '<td style="text-align:right;" >'.number_format($temp,0,',','.').'</td>';
                    ...
                    //totale diretto
                    echo '<tr style="border-top: 2px solid #777777;border-bottom: 2px solid #777777;">';
                        echo '<th style="text-align:left;height:40px;">Totale Personale diretto:</th>';
                        foreach ($this->reparti as $rep=>$r) {
                            
                            echo '<th style="text-align:right;';
                                if (isset($this->prodLinkRif['common'][$rep]) && $this->prodLinkRif['common'][$rep]['direttoNOsum']) {

                                    echo 'color:red;" >'.number_format($this->griglia[$rep]['diretto'],0,'','.');
                                }
                                else {
                                    echo '" >'.number_format($this->griglia[$rep]['diretto'],0,'','.');
                                }
                            echo '</th>';
                        }
                        echo '<th style="text-align:right;" >'.number_format($this->griglia['tot']['diretto'],0,'','.').'</th>';
                    echo '</tr>';
                    ...
                    echo '<tr style="background-color:#ffdf61;" >';
                        echo '<th style="text-align:left;height:20px;">2° Margine:</th>';
                       
                        foreach ($this->reparti as $rep=>$r) {
                            $this->griglia[$rep]['secondo']=$this->griglia[$rep]['primo']-$this->griglia[$rep]['diretto'];
                            echo '<th style="text-align:right;" >'.number_format($this->griglia[$rep]['secondo'],0,'','.').'</th>';
                        }
                        $this->griglia['tot']['secondo']=$this->griglia['tot']['primo']-$this->griglia['tot']['diretto'];
                        echo '<th style="text-align:right;" >'.number_format($this->griglia['tot']['secondo'],0,'','.').'</th>';
                    echo '</tr>';
                    ...
                    echo '<tr>';
                        ...
                        foreach ($this->reparti as $rep=>$r) {
                            ...
                            echo '<td style="text-align:right;" >'.number_format($costo,0,',','.').'</td>';
                            $temp+=$costo;
                            $this->griglia[$rep]['indiretto']+=$costo;
                        }
                        echo '<td style="text-align:right;" >'.number_format($temp,0,',','.').'</td>';
                        $this->griglia['tot']['indiretto']+=$temp;
                    echo '</tr>';

                    //totale indiretto
                    echo '<tr style="border-top: 2px solid #777777;border-bottom: 2px solid #777777;">';
                        echo '<th style="text-align:left;height:40px;">Totale Personale indiretto:</th>';
                        foreach ($this->reparti as $rep=>$r) {
                            
                            echo '<th style="text-align:right;';
                                if (isset($this->prodLinkRif['common'][$rep])) {
                                    echo 'color:violet;" >('.number_format($this->griglia[$rep]['indiretto'],0,'','.').')';
                                }
                                else {
                                    echo '" >'.number_format($this->griglia[$rep]['indiretto'],0,'','.');
                                }
                            echo '</th>';
                        }
                        echo '<th style="text-align:right;" >'.number_format($this->griglia['tot']['indiretto'],0,'','.').'</th>';
                    echo '</tr>';

                    //#####################################################################
                    //TERZO MARGINE DI CONTRIBUZIONE
                    //#####################################################################

                    echo '<tr style="background-color:#ffdf61;" >';
                        echo '<th style="text-align:left;height:20px;">3° Margine:</th>';
                       
                        foreach ($this->reparti as $rep=>$r) {
                            $this->griglia[$rep]['terzo']=$this->griglia[$rep]['secondo']-$this->griglia[$rep]['indiretto'];
                            echo '<th style="text-align:right;" >'.number_format($this->griglia[$rep]['terzo'],0,'','.').'</th>';
                        }
                        $this->griglia['tot']['terzo']=$this->griglia['tot']['secondo']-$this->griglia['tot']['indiretto'];
                        echo '<th style="text-align:right;" >'.number_format($this->griglia['tot']['terzo'],0,'','.').'</th>';
                    echo '</tr>';
                    ...
                    echo '<tr style="border-top: 2px solid black;border-bottom: 2px solid black;">';
                        ...
                        foreach ($this->reparti as $rep=>$r) {
                            echo '<th style="text-align:right;';
                                if (!$tempColor[$rep]) {
                                    echo 'color:orange;" >'.number_format($this->griglia[$rep]['fissi'],0,'','.');
                                }
                                else {
                    ...
                    echo '<tr style="background-color:#ffdf61;" >';
                        ...
                        foreach ($this->reparti as $rep=>$r) {
                            $this->griglia[$rep]['finale']=$this->griglia[$rep]['terzo']-$this->griglia[$rep]['fissi'];
                            echo '<th style="text-align:right;" >'.number_format($this->griglia[$rep]['finale'],0,'','.').'</th>';
                        }
                        $this->griglia['tot']['finale']=$this->griglia['tot']['terzo']-$this->griglia['tot']['fissi'];
                        echo '<th style="text-align:right;" >'.number_format($this->griglia['tot']['finale'],0,'','.').'</th>';
                    echo '</tr>';
                    ...
                    echo '<tr style="background-color:#aecce7;border-top:2px solid black;" >';
                        ...
                        $this->griglia['tot']['pareggio']=$this->griglia['tot']['oneri']['costi']+$this->griglia['tot']['diretto']+$this->griglia['tot']['indiretto']+$this->griglia['tot']['fissi']+$this->griglia['tot']['variabili'];
                        foreach ($this->reparti as $rep=>$r) {
                            $this->griglia[$rep]['pareggio']=$this->griglia[$rep]['oneri']['costi']+$this->griglia[$rep]['diretto']+$this->griglia[$rep]['indiretto']+$this->griglia[$rep]['fissi']+$this->griglia[$rep]['ric']['gesmag']+$this->griglia[$rep]['lis']['costo']+$this->griglia[$rep]['lis']['excos'];
                    ...
                    echo '<tr style="background-color:#aecce7;" >';
                        ...
                        //if (!isset($this->prodLinkRif['common'][$rep])) {
                            $this->griglia['tot']['risultato']=$this->griglia['tot']['oneri']['valore']+$this->griglia['tot']['fatt'];
                        //}
                        foreach ($this->reparti as $rep=>$r) {
                            $this->griglia[$rep]['risultato']=$this->griglia[$rep]['oneri']['valore']+$this->griglia[$rep]['fatt'];
                    ...
                    //################################################################
                    //BONUS e AZIONI PROMOZIONALI
                    foreach ($this->catBonus as $cat=>$b) {
                        ...
                        echo '<tr style="background-color:#aecce7;" >';
                            ...
                            foreach ($this->reparti as $rep=>$r) {
                                ...
                                $val=(isset($this->bonus[$rep][$cat]['valore']))?$this->bonus[$rep][$cat]['valore']:0;
                                echo '<td style="text-align:right;">'.number_format($val,0,'','.').'</td>';

                                $this->totBonus[$rep]+=$val;
                                $temp+=$val;
                            }

                            echo '<td style="text-align:right;">'.number_format($temp,0,'','.').'</td>';
                            $this->totBonus['totale']+=$temp;
                    ...
                    echo '<tr>';
                        ...
                        foreach ($this->reparti as $rep=>$r) {
                            ...
                            echo '<th style="text-align:right;';
                                if (isset($this->prodLinkRif['common'][$rep])) {
                                    echo 'color:violet;" >('.number_format($this->griglia[$rep]['risultato']-$this->griglia[$rep]['pareggio']+$this->totBonus[$rep],0,'','.').')';
                                }
                                else {
                        ...
                        echo '<th style="text-align:right;" >'.number_format($this->griglia['tot']['risultato']-$this->griglia['tot']['pareggio']+$this->totBonus['totale'],0,'','.').'</th>';
                    echo '</tr>';
