...
class vw_rev {
	...
	function __construct($report,$input,$data,$modo) {
		...
		//MYSQL
			//trova elementi
			$query='SELECT odl,data,intestazione,telaio,km,consegna,tel1,tel2,mail FROM test_app WHERE MONTH(data)='.(int)substr($data,4,2).' AND YEAR(data)='.(int)substr($data,0,4);
	...
	function intest() {
		echo '<div style="margin-bottom:10px;">';
			...
			echo substr($this->data,4,2)."/".substr($this->data,0,4);
			echo '</span>';
		echo '</div>';
	}
...
$lista=new vw_rev($report,$input,$data,$modo);
