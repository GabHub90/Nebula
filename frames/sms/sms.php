...
<script type="text/javascript" src="outsms.js"></script>
...
<body onload="calcolaLunghezza();">
	<div class="cornice">SMS - Service
	</div>
	<!--scelta dei modelli-->
	<div id="modelli">
		<?php
			$query="SELECT * FROM modelli WHERE stato=1";
			...
			while ($row=mysqli_fetch_array($modelli)) {
				...
				//$current_encoding = mb_detect_encoding($row[corpo], 'auto');
				//$row[corpo] = iconv('', 'UTF-8', $row[corpo]);
				$row[corpo]=addslashes($row[corpo]);
				echo"modelli[$x]=new Array('$row[nome]','".utf8_encode($row[corpo])."','$row[data]','$row[ora]','$row[hr]','$row[min]','$row[extra]');";
				echo"</script>";
				$x += 1;
			}
		?>
								...
								<tr>
									<td>
										<select id="ext" name="ext" class="chr" form="messaggio" onchange="modificaExtra();">
											<?php
												$query="SELECT * FROM extra WHERE stato=1 ORDER BY ID ASC";
							...
							<div id="col">
								<?php
									mysqli_select_db($db,$db2_name)
										or die ("Errore nella selezione del database. Verificare i parametri nel file config.inc.php");
									$query="SELECT nome,telefono,marche.marca AS mc FROM collaboratori,marche,funzioni WHERE collaboratori.ID=collaboratore AND programma=10 AND collaboratori.marca=marche.ID ORDER BY mc";
									$col=mysqli_query($db,$query);
									while ($row=mysqli_fetch_array($col)) {
										if ($row[mc]=="vw") $img="vw.jpg";
										if ($row[mc]=="audi") $img="audi.jpg";
										if ($row[mc]=="porsche") $img="porsche.jpg";
