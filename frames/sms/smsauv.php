...
<body onload="calcolaLunghezza();">
	<div class="corniceauv">SMS - Vendita Audi
	</div>
	<!--scelta dei modelli-->
	<div id="modelli">
		<?php
			$query="SELECT * FROM modelliauv WHERE stato=1";
								...
								<tr>
									<td>
										<select id="ext" name="ext" class="chr" form="messaggio" onchange="modificaExtra();">
											<?php
												$query="SELECT * FROM extraauv WHERE stato=1 ORDER BY ID ASC";
							...
							<div id="col">
								<?php
									mysqli_select_db($db,$db2_name)
										or die ("Errore nella selezione del database. Verificare i parametri nel file config.inc.php");
									$query="SELECT nome,telefono,marche.marca AS mc FROM collaboratori,marche,funzioni WHERE collaboratori.ID=collaboratore AND programma=12 AND collaboratori.marca=marche.ID ORDER BY mc";
