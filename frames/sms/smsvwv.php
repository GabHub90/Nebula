...
<body onload="calcolaLunghezza();">
	<div class="cornicevwv">SMS - Vendita Volkswagen
	</div>
	<!--scelta dei modelli-->
	<div id="modelli">
		<?php
			$query="SELECT * FROM modellivwv WHERE stato=1";
							...
							<div id="col">
								<?php
									mysqli_select_db($db,$db2_name)
										or die ("Errore nella selezione del database. Verificare i parametri nel file config.inc.php");
									$query="SELECT nome,telefono,marche.marca AS mc FROM collaboratori,marche,funzioni WHERE collaboratori.ID=collaboratore AND programma=13 AND collaboratori.marca=marche.ID ORDER BY mc";
									$col=mysqli_query($db,$query);
									while ($row=mysqli_fetch_array($col)) {
										if ($row[mc]=="vw") $img="vw.jpg";
										if ($row[mc]=="tutte") $img="star.png";
										if ($row[mc]=="amm") $img="amm.jpg";
